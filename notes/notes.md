...
- js原型链相关
  - new 操作执行过程
    - 创建一个新对象
    - 将构造函数的作用域指定为新对象并执行，然后就会通过 this 绑定属性，原型上的属性或者方法通过原型链访问
    - 返回新对象
...
- https加密流程：
  ...
  - Client Key Exchange：客户端将这个 PreMaster Key 传给服务端，服务端再用自己的私钥解出这个 PreMaster Key 得到客户端生成的 Random3。至此，客户端和服务端都拥有 Random1 + Random2 + Random3，两边再根据同样的算法就可以生成一份秘钥，握手结束后的应用层数据都是使用这个秘钥进行对称加密。为什么要使用三个随机数呢？这是因为 SSL/TLS 握手过程的数据都是明文传输的，并且多个随机数种子来生成秘钥不容易被暴力破解出来
  - 最后客户端和服务端互发一个加密的事件消息，如果双方都能解析出来，说明握手成功
...
- 域名解析类型
  ...
  - NS：指定进行域名解析的服务器
- dns解析
  - .com是顶级域名，后面依次是二级、三级域名，一般是先查顶级域名的NS，然后查二级域名的NS，依次向下查找，具体示例https://blog.csdn.net/crazw/article/details/8986504
  - Baidu.com就是一个顶级域名，而www.baidu.com却不是顶级域名，他是在baidu.com 这个域里的一叫做www的主机。比如a.www.baidu.com，在这个网址中，www.baidu.com变成了一个二级域而不是一台主机，主机名是a。
- 函数式编程
  - 简单理解：用函数来组织过程、业务，而面向对象编程基于对象来组织（函数式编程关心数据的映射（用函数来表达映射关系），命令式编程关心解决问题的步骤）
  - 特点：
    - 函数是一等公民：指的是函数与其他数据类型一样，处于平等地位，可以赋值给其他变量，也可以作为参数，传入另一个函数，或者作为别的函数的返回值
    - 只用"表达式"，不用"语句"，语句是做一个操作但没有返回值，表达式一定有返回值
    - 无副作用：函数要保持独立，所有功能就是返回一个新的值，没有其他行为，尤其是不得修改外部变量的值。
...
- JSONP 实现：
  ...
  ```
- linux 进程
  - 在linux内核中，使用了一个task_struct（描述符）结构体来描述进程信息，并且通过双向循环链表将所有的进程连接起来。一个task_struct描述了进程打开文件信息，状态信息，地址空间信息，挂起的信号，进程的状态等等。
  - 线程是进程中活动的对象，每个线程都拥有一个独立的程序计数器，进程炸，和一组进程寄存器
  - 内核调度的对象是线程，而不是进程
  - 在linux系统中通过fork()来复制现有进程的资源和创建一个新的进程。随后调用exec这组函数创建自己的地址空间最后载入要运行的程序,在linux中fork其实是clone系统调用来实现。程序执行完毕后使用exit退出执行，父进程调用wait或waitpid来等待子进程结束，并回收其资源。
  - 上下文：进程上下文其实就是进程的当前状态，一组状态数据而已，在进程切换的时候，保存当前进程的数据，载入要切换进程的数据，这样就可以实现进程的切换。状态数据通常包含CPU所有寄存器的值，进程的状态，堆栈中的内容等等。
  - 进程树：初始进程 init，所有进程都fork自init或者init的子孙进程。如果某个进程的父进程意外退出，该进程的父进程会变成init
  - 僵尸进程：子进程执行完，但父进程没有回收
  - 孤儿进程：父进程死，但子进程还在运行，这时孤儿会变成 init 的子进程，init 会负责回收子进程
  - 经常调度：不同类型的线程具有不同优先级。进程调度算法：时间片轮转调度算法（round-robin）、优先权调度算法、FIFO（先进先出）调度算法
  - 进程间通讯：
    - 管道：管道实际上只是内存中的一个固定大小的缓冲区，将管道前端进程的标准输出重定向到管道后端进程的标准输入中，一头读，一头写
